...
const removeLocalData = async (key: string) => {
  try {
    ...
    console.log(
      `Erro ao remover dados (key: ${key}) do LocalStorage: ${error}`,
    );
  }
};


const removeFromFavoritosByKeyAndValue = async (
  key: string,
  codigoLivro: any,
) => {
  ...
  try {
    ...
    console.log(
      `Erro ao remover dados (key: ${key}) com a valor do codigo do livro ${codigoLivro} do LocalStorage: ${error}`,
    );
  }
};
